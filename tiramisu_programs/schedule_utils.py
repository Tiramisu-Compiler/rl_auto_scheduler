...
class ScheduleUtils:
    ...
    def get_representation(cls, program_annot):
        ...
        for index, comp_name in enumerate(ordered_comp_list):
            ...
            iterators_repr.extend(
                [0] * iterator_repr_size * 2 * (max_depth - len(comp_dict["iterators"]))
            )
    ...
    def sched_str(cls, sched_str, id, params, comp_indic):
        if id in range(28):
            sched_str += (
                "I(L"
                + str(params["first_dim_index"])
                + ",L"
                + str(params["second_dim_index"])
                + ")"
            )
        else:
            if id in range(28, 41):
                if params["tiling_depth"] == 2:
                    sched_str += (
                        ...
                        + str(params["second_dim_index"])
                        + ","
                        + str(params["first_factor"])
                        + ","
                        + str(params["second_factor"])
                        + ")"
                    )
                else:
                    sched_str += (
                        ...
                        + str(params["second_dim_index"])
                        + ",L"
                        + str(params["third_dim_index"])
                        + ","
                        + str(params["first_factor"])
                        + ","
                        + str(params["second_factor"])
                        + ","
                        + str(params["third_factor"])
                        + ")"
                    )
            else:
                if id in range(41, 44):
                    for comp in params:
                        sched_str += (
                            "U(L"
                            + str(params[comp]["dim_index"])
                            + ","
                            + str(params[comp]["unrolling_factor"])
                            + ",C"
                            + str(comp_indic[comp])
                            + ")"
                        )
    ...
    def get_orig_tree_struct(cls, program_json, root_iterator):
        tree_struct = {
            "loop_name": root_iterator,
            "computations_list": program_json["iterators"][root_iterator][
                "computations_list"
            ][:],
            "child_list": [],
        }
    ...
    @classmethod
    def update_iterators(
        cls, id, it_list, action_params, added_iterators, comp_indic_dict
    ):
        for comp in it_list:
            if id in range(28):
                tmp = it_list[comp][action_params["first_dim_index"]]
                it_list[comp][action_params["first_dim_index"]] = it_list[comp].pop(
            ...
            if id in range(28, 41):
                ...
                if action_params["tiling_depth"] == 2:
                    while i > depth_2:
                        if (
                            ...
                            and action_params["tiling_loop_2"]
                        ):
                            it_list[comp][i + 2] = it_list[comp][i]
                        elif (
                            action_params["tiling_loop_1"]
                            or action_params["tiling_loop_2"]
                        ):
                    ...
                    if action_params["tiling_depth"] == 3:
                        ...
                        while i > depth_3:
                            if (
                                action_params["tiling_loop_1"]
                                and action_params["tiling_loop_2"]
                                and action_params["tiling_loop_3"]
                            ):
                ...
                if action_params["tiling_depth"] == 2:
                    if (
                        ...
                        it_list[comp][depth_1]["upper_bound"] = (
                            it_list[comp][depth_1]["upper_bound"]
                            / action_params["first_factor"]
                        )
                        it_list[comp][depth_1 + 2] = {}
                        it_list[comp][depth_1 + 2]["iterator"] = "{}_1".format(
                            it_list[comp][depth_1]["iterator"]
                        )
                        it_list[comp][depth_1 + 2]["lower_bound"] = it_list[comp][
                            depth_1
                        ]["lower_bound"]
                        it_list[comp][depth_1 + 2]["upper_bound"] = action_params[
                            "first_factor"
                        ]

                        added_iterators.append(it_list[comp][depth_1 + 2]["iterator"])

                        it_list[comp][depth_2]["upper_bound"] = (
                            it_list[comp][depth_2]["upper_bound"]
                        ...
                        it_list[comp][depth_2 + 2]["lower_bound"] = it_list[comp][
                            depth_2
                        ]["lower_bound"]
                        it_list[comp][depth_2 + 2]["upper_bound"] = action_params[
                            "second_factor"
                        ]
                        ...
                        if action_params["tiling_loop_1"]:
                            ...
                            it_list[comp][depth_2 + 1]["iterator"] = "{}_1".format(
                                it_list[comp][depth_2]["iterator"]
                            )
                            it_list[comp][depth_2 + 1]["lower_bound"] = it_list[comp][
                                depth_2
                            ]["lower_bound"]
                            it_list[comp][depth_2 + 1]["upper_bound"] = action_params[
                                "second_factor"
                            ]
                    ...
                    if (
                        ...
                        it_list[comp][depth_1 + 3]["lower_bound"] = it_list[comp][
                            depth_1
                        ]["lower_bound"]
                        it_list[comp][depth_1 + 3]["upper_bound"] = action_params[
                            "first_factor"
                        ]
                        ...
                        it_list[comp][depth_2 + 3]["lower_bound"] = it_list[comp][
                            depth_2
                        ]["lower_bound"]
                        it_list[comp][depth_2 + 3]["upper_bound"] = action_params[
                            "second_factor"
                        ]

                        added_iterators.append(it_list[comp][depth_2 + 3]["iterator"])

                        it_list[comp][depth_3]["upper_bound"] = (
                            it_list[comp][depth_3]["upper_bound"]
                        ...
                        it_list[comp][depth_3 + 3]["lower_bound"] = it_list[comp][
                            depth_3
                        ]["lower_bound"]
                        it_list[comp][depth_3 + 3]["upper_bound"] = action_params[
                            "third_factor"
                        ]
                        ...
                        it_list[comp][depth_3 + 2]["lower_bound"] = it_list[comp][
                            depth_3
                        ]["lower_bound"]
                        it_list[comp][depth_3 + 2]["upper_bound"] = action_params[
                            "third_factor"
                        ]
                        ...
                        if action_params["tiling_loop_1"]:
                            ...
                            it_list[comp][depth_3 + 1]["iterator"] = "{}_1".format(
                                it_list[comp][depth_3]["iterator"]
                            )
                            it_list[comp][depth_3 + 1]["lower_bound"] = it_list[comp][
                                depth_3
                            ]["lower_bound"]
                            it_list[comp][depth_3 + 1]["upper_bound"] = action_params[
                                "third_factor"
                            ]
                ...
                it_list[comp][action_params["dim_index"]]["upper_bound"] = (
                    it_list[comp][action_params["dim_index"]]["upper_bound"]
                    / action_params["unrolling_factor"]
                )
                ...
                l1_upper_bound = (
                    l1_lower_bound
                    + abs(action_params["first_factor"]) * l1_extent
                    + abs(action_params["second_factor"]) * l2_extent
                )
                l2_upper_bound = (
                    (l1_extent * l2_extent) / (l1_upper_bound - l1_lower_bound)
                ) + 1
                ...
                it_list[comp][action_params["dim_index"]]["lower_bound"] = it_list[
                    comp
    ...
    def optimlist_to_str(cls, optim_list):
        ...
        comp_names = list(set([comp for optim in optim_list for comp in optim.comps]))
