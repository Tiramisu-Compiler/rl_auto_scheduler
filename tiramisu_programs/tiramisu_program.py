...
$buffers_init$
    ...
    def get_program_annotations(self):
        ...
        std::ofstream out("'''
                + self.func_folder
                + self.name
                + """_program_annotations.json");
        out << program_json;
        out.close();
        """
            )
            get_json_prog = self.original_str.replace(
                self.code_gen_line, get_json_lines
            )
            ...
            with open(
                self.func_folder + self.name + "_program_annotations.json", "r"
            ) as f:
    ...
    def check_legality_of_schedule(self, optims_list, comps=None, first_comp=None):
...
"""
    ...
    is_legal &= loop_unrolling_is_legal("""
                    + str(optim.params_list[comps[0]][0])
                    + """, {"""
    ...
    def call_solver(self, comp, params):
        ...
        if os.path.isfile(lc_file):
            ...
            to_replace = re.findall(r"(std::ofstream out(?s:.)+)return", original_str)[
                0
            ]
    ...
    """
        ...
        log_message = "Solver results for: computation {}".format(comp) + " ".join(
            [p for p in params]
        )
    ...
    def evaluate_schedule(
        self, optims_list, cmd_type, nb_executions, initial_exec_time=None
    ):
        ...
        if CPP_File.compile_and_run_tiramisu_code(
            ...
            try:
                ...
                return (
                    10
                    * nb_executions
                    * (initial_exec_time if initial_exec_time else 1.0)
                )
        else:
            raise InternalExecException
    ...
    def get_measurements(self, cmd_type, nb_executions, initial_exec_time):
        ...
        log_message_cmd = (
            'printf "Running wrapper nb_exec = '
            + str(nb_executions)
            + '\n">> ${FUNC_DIR}log.txt'
        )
        run_wrapper_cmd = (
        ...
        s_time = time.time()
        failed = CPP_File.launch_cmd(
            run_wrapper_cmd, self.file_path, cmd_type, nb_executions, initial_exec_time
        )
    ...
    """
        wrapper_cpp_code = self.wrapper_cpp_template.replace("$func_name$", self.name)
