...
class Tiramisu_Program():
    ...
    def check_legality_of_schedule(self, optims_list, comps=None, first_comp=None): #Optims_list should be the order of which they should be applied # works only for single comp
...
'''
                ...
                for comp in comps:
                    legality_check_lines += '''
        is_legal &= loop_unrolling_is_legal('''+str(optim.params_list[comp][0])+''', {&'''+comp+'''});
    '''
                    legality_check_lines += optim.tiramisu_optim_str+'\n' #not sure if this line is necessary
