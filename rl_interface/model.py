...
class TiramisuModelMult(TorchModelV2, nn.Module):
    def __init__(
        ...
        for size in shared_layer_sizes:
            comp_embd_layers.extend(
                [nn.Linear(prev_layer_size, size), nn.Dropout(0.02), nn.ELU()]
            )
        ...
        self.comps_lstm = nn.LSTM(
            shared_layer_sizes[-1], embedding_size, batch_first=True
        )
    ...
    def forward(self, input_dict, state, seq_len):
        ...
        computations_indices = input_dict["obs_flat"]["computations_indices"][:][:][0][
            0
        ]
        ...
        try:
            ...
            prog_embedding = self.get_hidden_state(
                prog_tree, comps_embeddings, loops_tensor
            )
        except:
        ...
        # prediction layer
        self._features = self._prediction_layers(
            prog_embedding.view(prog_embedding.shape[0], -1)
        )
