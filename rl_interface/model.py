...
class TiramisuModelMult(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config,
        ...
        self.comps_lstm = nn.LSTM(shared_layer_sizes[-1],
                                  embedding_size,
                                  batch_first=True)
        self.nodes_lstm = nn.LSTM(shared_layer_sizes[-1],
                                  embedding_size,
                                  batch_first=True)
    ...
    def forward(self, input_dict, state, seq_len):
        ...
        computations_indices = input_dict["obs_flat"][
            "computations_indices"][:][:][0][0]
        loop_index = 0
        prog_embedding = self.get_hidden_state(input_dict["obs_flat"],
                                               child_list, has_comps,
                                               computations_indices,
                                               comps_embeddings, loops_tensor,
                                               loop_index)
    ...
    def get_hidden_state(self, input_dict, child_list, has_comps,
                         computations_indices, comps_embeddings, loops_tensor,
                         loop_index):
        nodes_list = []

        if len(child_list) != 0 and not all(element == 0
                                            for element in child_list):
            for child in child_list:
                tmp = math.floor(child)
                childs = input_dict["child_list"][:][:][0][tmp]
                ## print('children',childs)
                computations_indices = input_dict[
                    "computations_indices"][:][:][0][tmp]
                #computations_indices=torch.tensor(computations_indices).to(train_device)
                childs = childs[childs != -1]
                if len(childs) != 0:
                    nodes_list.append(
                        self.get_hidden_state(input_dict, childs, has_comps,
                                              computations_indices,
                                              comps_embeddings, loops_tensor,
                                              tmp))
            ...
            if (nodes_list != []):
                ...
                if nodes_tensor.shape[1] != 180:
                    nodes_tensor = nodes_tensor.reshape(
                        int(nodes_tensor.shape[1] / 180), 180)
                lstm_out, (nodes_h_n,
                           nodes_c_n) = self.nodes_lstm(nodes_tensor)
            ...
            if has_comps[loop_index] == 1:
                computations_indices = computations_indices[
                    computations_indices != -1].int()
                comps_embeddings = comps_embeddings.reshape(5, 180)
                selected_comps_tensor = torch.index_select(
                    comps_embeddings, 0, computations_indices)
                lstm_out, (comps_h_n,
            ...
            selected_loop_tensor = torch.index_select(
                loops_tensor[-1, :, :], 0,
                torch.tensor(loop_index).to(train_device))
